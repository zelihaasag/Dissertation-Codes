...
class Monopolar_VAE(nn.Module):
    ...
    def decode(self, amps, sampled_n_loc, ch_locs, exp_ids):
        distances = self.getTensorDistances(sampled_n_loc, ch_locs)
        
        ampslist= torch.sum(amps[:,:,0], dim=1)
        
        #Optimizing alpha values
        alpha = torch.sum(distances * torch.tile(ampslist, (25,1)).T) / torch.sum(torch.square(distances), dim=1)
        recon_amps = torch.tile(alpha, (25,1)).T / distances
        return recon_amps
    ...
    def forward(self, amps, waveforms, ch_locs, exp_ids):
        ...
        recon_amps = self.decode(amps, sampled_n_loc, ch_locs, exp_ids)
